...
func initLogging() (*os.File, error) {
	...
	logDir := filepath.Join(tempDir, "epub-spool")
...
func main() {
	...
	if len(os.Args) < 2 {
		...
		msgText := "Voulez-vous convertir un document existant (PDF, PowerPoint, Word, Excel, HTML, Texte) en EPUB ?\n\n" +
		...
		ret := walk.MsgBox(nil, "Print to EPUB", msgText, walk.MsgBoxYesNoCancel|walk.MsgBoxIconQuestion)
		if ret == walk.DlgCmdYes {
			dlg := &walk.FileDialog{
				Title:  "Sélectionnez le document à convertir en EPUB",
	...
	dlg := &walk.FileDialog{
		Title:          "Save Document as EPUB",
		Filter:         "EPUB File (*.epub)|*.epub|All Files (*.*)|*.*",
	...
	if err != nil {
		...
		log.Printf("main: Compiling EPUB to target = %s", dlg.FilePath)
		err = converter.ConvertToEpub(mdContent, defaultName, dlg.FilePath)
		if err != nil {
			log.Printf("main: Error writing output file: %v", err)
			walk.MsgBox(nil, "Save Error", fmt.Sprintf("Could not compile the EPUB file:\n%v", err), walk.MsgBoxIconError)
		} else {
